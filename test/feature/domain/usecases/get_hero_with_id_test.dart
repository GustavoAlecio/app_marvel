...
void main() {
  ...
  test('Quando realizar a query,deve retornar um HeroEntity dentro de um Right',
      () async {
    //
    when(() => connectivity.isOnline())
        .thenAnswer((invocation) => Future.value(true));
    when(() => repository.getHeroWithId(0))
        .thenAnswer((_) => Future.value(Right(entity)));
    final usecase = GetHeroWithID(
        repository: repository, connectivityService: connectivity);
    ...
    expect(resultFold, isA<HeroEntity>());
  });

  test('Quando realizar a query, deve retornar um Failure', () async {
    when(() => connectivity.isOnline())
        .thenAnswer((invocation) => Future.value(true));
    when(() => repository.getHeroWithId(0)).thenAnswer((_) => Future.value(
